...
This repository provides a self-learning notebook series for starting to use Python for research. It first covers the core building blocks of programming with Python that are also illustrated in the mindmap: variables, data types, control flow, and functions. The notebooks then dive into the very basics of (A) Natural Language Processing (NLP) for qualitative research, and (B) working with quantitative data in research, including working with tabular data, basic statistics, and simple analysis workflows.
